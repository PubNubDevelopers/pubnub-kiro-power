...
The package includes a local MCP configuration for the published PubNub MCP server. The MCP server gives Kiro access to PubNub documentation and, when users provide their own credentials, PubNub account and keyset operations.

## Contents

- `POWER.md` defines the Power identity.
- `mcp.json` configures PubNub MCP with environment-variable placeholders only.
...
Set real values in your shell, local Kiro MCP settings, or another secret store. Never commit real PubNub keys or service integration credentials.

`PUBNUB_API_KEY` unlocks account and keyset management operations. Publish and subscribe examples can use `PUBNUB_PUBLISH_KEY` and `PUBNUB_SUBSCRIBE_KEY` for realtime operations. `PUBNUB_USER_ID` is optional and identifies the SDK user for local realtime calls.
